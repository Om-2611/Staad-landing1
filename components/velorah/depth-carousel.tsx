...
const DepthCarousel = ({
  ...
  /*
   * Mirror the live props into refs so the pointer/wheel handlers and the GSAP
   * tween always read current values without re-subscribing. Written in an
   * effect rather than straight through during render: React forbids touching
   * a ref's `current` while rendering, and this effect is declared first so the
   * config is populated before any of the effects below run `layout`.
   */
  useEffect(() => {
